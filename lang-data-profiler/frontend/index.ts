import { FastAPIRequest, DataResponse } from "./dataRequest.js"
...
const insertSampleText = (event:any) => {
    const clicked = event.target.innerText;
    switch(clicked){
        case "Sample text 1":
            $("#inputText").val("").val(SAMPLE1);
            break;
        
        case "Sample text 2":
            $("#inputText").val("").val(SAMPLE2);
            break;

        case "Sample text 3":
            $("#inputText").val("").val(SAMPLE3);
            break;
    }
}
...
const updateResultDiv = (data:DataResponse) => {
    if (data){
        $("#json-results").html("");
        const textDiv = document.createElement("div");
        textDiv.className = "col border overflow-scroll h-100";
        textDiv.innerText = JSON.stringify(data);
        $("#json-results").append(textDiv);
    }
...
const attachFormHandler = (): void => {
    ...
    $("#input-form").on("submit", async function(event:any) {
        ...
        updateResultDiv(request.data)

        
    })
}

const attachSampleButtonHandler = ():void => {
    $(".sample-btn").on("click", insertSampleText)
}



attachFormHandler();
attachSampleButtonHandler();
